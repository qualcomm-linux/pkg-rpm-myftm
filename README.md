...
This branch contains the CentOS Stream 10 RPM packaging for myftm from a prebuilt payload tarball.
...
| Package | myftm |
| Version | 1.0.0 |
| Source | myftm-prebuilt-1.0.0.tar.gz |
...
Local validation can be run with qcom-rpm-utils:
    ...
    /path/to/qcom-rpm-utils/scripts/build-rpm.sh \
      --tarball /path/to/myftm-prebuilt-1.0.0.tar.gz \
      --spec myftm.spec \
